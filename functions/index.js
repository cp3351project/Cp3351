...
exports.createSampleData = functions.https.onCall(
  async (data, context) => {
    ...
    const { uid: authId5 } = await admin.auth().createUser({ email: "gehad@gehad.com", password: "gehadgehad" })
    ...
    const result5 = await db.collection('users').doc(authId5).set({ name: "gehad", role: "Tester" })
