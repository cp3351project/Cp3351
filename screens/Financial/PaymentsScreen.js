...
export default function PaymentsScreen() {
  ...
  useEffect(() => {
    try {
      db.Payments.listenAll(setPayments);      
    } catch (error) {
      alert(error)
    }
    return () => {
  ...
  return (
    <View>
      <ScrollView>
        {payments.map((paymentItem, i) => (
           paymentItem.Descion === false && <Pressable key={i}>
            {
              <ListItem
              ...
              >
                <ListItem.Content>
                  <ListItem.Title style={{fontSize:20}}>Buyer: {paymentItem.BuyersName}</ListItem.Title>
                  <ListItem.Subtitle>Payment total: {paymentItem.total}</ListItem.Subtitle>
